...
export const organizations = pgTable('organizations', {
    ...
    editorialProfile: jsonb('editorial_profile').$type().notNull().default({}),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
});
...
export const mcpTokens = pgTable('mcp_tokens', {
    ...
    canAccessUnscoped: boolean('can_access_unscoped').notNull().default(false),
    organizationIds: jsonb('organization_ids').$type().notNull().default([]),
...
export const designTemplateRuns = pgTable('design_template_runs', {
    ...
    outputSpec: jsonb('output_spec').$type().notNull().default({}),
    metadata: jsonb('metadata').$type().notNull().default({}),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
...
export const mktContentItems = pgTable('mkt_content_items', {
    ...
    supportedChannels: jsonb('supported_channels').$type().notNull().default([]),
...
export const socialProviderApps = pgTable('social_provider_apps', {
    ...
    scopesRequested: jsonb('scopes_requested').$type().notNull().default([]),
...
export const mktAdMetricSnapshots = pgTable('mkt_ad_metric_snapshots', {
    ...
    currencyCode: text('currency_code').notNull().default('MXN'),
    rawMetrics: jsonb('raw_metrics').$type().notNull().default({}),
...
export const searchConsoleMetricSnapshots = pgTable('search_console_metric_snapshots', {
    ...
    uniqueMetricIdx: uniqueIndex('search_console_metric_snapshots_unique_idx').on(table.propertyId, table.dataDate, table.searchType, table.page, table.query, table.country, table.device),
}));
export const searchConsoleDailySnapshots = pgTable('search_console_daily_snapshots', {
    ...
    uniqueDailySnapshotIdx: uniqueIndex('search_console_daily_snapshots_unique_idx').on(table.propertyId, table.dataDate, table.searchType),
}));
export const searchConsoleUrlInspections = pgTable('search_console_url_inspections', {
    ...
    rawResult: jsonb('raw_result').$type().notNull().default({}),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
