...
function buildDesignTemplateMcpPrompt(run: AgentRunRecord) {
  ...
  return [
    ...
    organizationProject === 'ardia' ? 'Ardia-specific hard contract: the Ardia buyer landing, Pach legacy ardia-one-pager, and Universo aBanza onboarding decks are canonical. Use organizationDesignSystem.metadata.requiredDesignContract as a QA checklist before saving. Do not drift into generic executive decks, generic SaaS cards, blue/purple gradients, neon/glass/bokeh panels, large serif primary titles, fake square logos, opaque red panels, or one long scrolling document. If the user asks for new content or broad visual changes, satisfy the request inside Ardia quiet-minimalist language unless they explicitly ask to replace the organization design system. Use Inter Tight 200 display titles, Geist Mono labels, one emotionally important inline Instrument Serif italic vermilion title phrase, the real Ardia mark/assets, hairlines, whitespace, quiet product/data surfaces, recurring low-area vermilion signals, the allowed subtle off-canvas vermilion radial glow, and fixed-size slide components.' : null,
